...
# let's remove seqlength informatio
seqlengths(gene) = NA
great(gr, gs, gene)
```
...
3. Contruct extended TSS by hand


```r
et = extendTSS(gene)
et = extendTSS(gene, gl = c("chr1" = 10, "chr2" = 20, ...))
et = extendTSS(gene, genome = "hg19")
```

Then `et` must be specified with the `extended_tss` argument.
...
Or only in a set of pre-defined background regions:
